...
class MultiwayQwen2_5VLForConditionalGenerationForMultiwayTraining(Qwen2_5_VLForConditionalGenerationForMultiwayTraining):
    ...
    def generate_random_selection_mask(
        source_mask: torch.Tensor,    # 输入的原始掩码，例如 attention_mask (bsz, seq_len)
        target_value_in_source: int,  # 在 source_mask 中标记有效/可选位置的值 (例如 1)
        num_to_select_per_sequence: Union[List[int], Tuple[int, ...], torch.Tensor] # 每个序列希望选出的位置数量的列表/元组/张量
    ) -> torch.Tensor:                 # 返回一个布尔掩码 (bsz, seq_len)，True 表示被选中的位置
        """
        根据源掩码随机选择一定数量的有效位置，并生成一个新的选择掩码。
        每个序列的选择数量可以不同。

        参数:
            source_mask (torch.Tensor): 输入的二维张量 (batch_size, sequence_length)。
                                        通常是注意力掩码，其中1表示有效token，0表示padding。
            target_value_in_source (int): 在 source_mask 中指示哪些位置是可选的的值 (例如, 1)。
            num_to_select_per_sequence (Union[List[int], Tuple[int, ...], torch.Tensor]):
                                        一个可迭代对象 (列表、元组或一维PyTorch张量)，其长度
                                        必须等于 batch_size。每个元素指定了对应序列中
                                        希望选择的位置数量。所有数量必须为非负整数。

        返回:
            torch.Tensor: 与 source_mask 形状相同的布尔张量。
                          被随机选中的位置在返回的张量中标记为 True。
        """
        if not source_mask.dim() == 2:
            raise ValueError("source_mask 必须是二维张量 (batch_size, sequence_length)。")

        batch_size, _ = source_mask.shape

        # 验证 num_to_select_per_sequence
        if isinstance(num_to_select_per_sequence, torch.Tensor):
            if not num_to_select_per_sequence.dim() == 1:
                raise ValueError("如果 num_to_select_per_sequence 是一个张量, 它必须是一维的。")
            if not num_to_select_per_sequence.size(0) == batch_size:
                raise ValueError(
                    f"num_to_select_per_sequence 张量的长度 ({num_to_select_per_sequence.size(0)}) "
                    f"必须等于 batch_size ({batch_size})。"
                )
        elif isinstance(num_to_select_per_sequence, (list, tuple)):
            if not len(num_to_select_per_sequence) == batch_size:
                raise ValueError(
                    f"num_to_select_per_sequence 列表/元组的长度 ({len(num_to_select_per_sequence)}) "
                    f"必须等于 batch_size ({batch_size})。"
                )
        else:
            raise TypeError(
                "num_to_select_per_sequence 必须是列表、元组或一维 PyTorch 张量。"
            )

        # 初始化选择掩码，所有值为 False (0)
        ...
        for i in range(batch_size):
            ...
            if not isinstance(current_num_to_select_for_seq, int) or current_num_to_select_for_seq < 0:
                raise ValueError(
                    f"num_to_select_per_sequence 中的所有元素必须是非负整数。 "
                    f"在序列 {i} 遇到值: {current_num_to_select_for_seq}。"
                )
            ...
            # 找到当前序列中所有等于 target_value_in_source 的位置的索引
